...
class SequentialRouteOptimization(ParallelRouteOptimation):
    '''
    Sequential Routing Optimization Class (inherits from ParallelRouteOptimization)
    ...
    Args: 
        ...
        ** num_stops: number of stops enforced in the system design 
        ...
        ** origin_node_id: route starting point (requires an osmnx node id)
    '''
    ...
    def __init__(
        ...
        # instantiate variables via parent class
        super().__init__(
            alpha = alpha, 
    ...
    def solve_problem(self):
        ...
        Consists of three types of constraints:
            *subtour/cycle elimination constraints. ensures the output route is in the form of a linked-list 
            *node-flow preservation (for evaluating the shortest path)
            *coverage enforcement along the route 
        '''
        ...
        M = 10000 #set M to be arbitrarily large (at least larger than the total number of nodes in-system)
        for i in node_list: 
    ...
    def build_route_parameters(self):
        ...
        self.results['Summary Statistics'] = {
            ...
            'Total System Time': vars_fac_df['cost'].max(),
            'Average User System Time': vars_fac_df['cost'].mean(),
            'Upper Bound User Time in System': vars_fac_df['cost'].max(),
            'Lower Bound User Time in System': vars_fac_df['cost'].min()
        }
